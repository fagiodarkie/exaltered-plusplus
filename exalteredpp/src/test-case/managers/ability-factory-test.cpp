...
TEST_CASE("Default Ability Factory")
{
  ...
  QList<character::ability_names::ability_enum> abilities_with_variations = { character::ability_names::CRAFT, character::ability_names::MANEUVER };

  SECTION("should retrieve all abilities")
  {
    for (character::ability_names::ability_enum abilityname : character::ability_names::ABILITY_NAME.keys())
      {
  ...
  SECTION("simple abilities should not have variations")
  {
    for (character::ability_names::ability_enum abilityname : abilities_with_variations)
      {
        character::ability_group gut = sut.get_ability_group(abilityname);
        REQUIRE(gut.get_abilities().size() > 1);
