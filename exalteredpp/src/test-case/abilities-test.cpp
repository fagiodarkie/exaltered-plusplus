...
TEST_CASE("Abilities")
{
  ...
  SECTION("should save ability in map")
  {
    ...
    sut.insert(WAR, character::ability_group(TEST_ABILITY_NAME, ability_category::COMBAT, {ability}));
    REQUIRE(sut.value(WAR).get_name() == TEST_ABILITY_NAME);
  }
  ...
  SECTION("should create correctly from JSON")
  {
    ...
    QJsonObject from;
    from[ABILITY_NAME.value(WAR)] = inner_obj;
    character::abilities sut;
    sut.read_from_json(from);
    REQUIRE(sut.value(WAR).get_name() == TEST_ABILITY_NAME);
    REQUIRE(sut.value(WAR).get_abilities().size() == inner_group.get_abilities().size());
    REQUIRE(sut.value(WAR).get_specialisations().size() == inner_group.get_specialisations().size());
  }
  ...
  SECTION("should write correctly to JSON")
  {
    character::abilities sut;
    character::ability ability(TEST_ABILITY_NAME, TEST_ABILITY_VALUE);
    sut.insert(WAR, character::ability_group(TEST_ABILITY_NAME, ability_category::COMBAT, {ability}, {character::specialisation("")}));
    QJsonObject from;
